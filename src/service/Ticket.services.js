...
class TicketService {
  ...
  async purchaseCart(cid, userEmail) {
    ...
    for (let i = 0; i < cart.products.length; i++) {
        ...
        if (producto.stock >= p.quantity) { // Verificamos si hay suficiente stock
            ...
            console.log(`No hay suficiente stock para el producto ${producto.title}. Se necesita ${p.quantity}, pero solo hay ${producto.stock}.`);
            noProcesados.push(p.product._id); // Agregar ID a productos no procesados
        }
    }
    ...
    cart.products = cart.products.filter(p => noProcesados.includes(p.product._id)); // Mantener solo los que no se compraron
